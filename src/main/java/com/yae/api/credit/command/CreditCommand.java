...
public class CreditCommand extends YAECommand {
    ...
    private boolean handleGradeCommand(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        creditService.getCreditGrade(player.getUniqueId()).thenAccept(grade -> {
            sender.sendMessage(MessageUtils.info("您的信用等级信息:"));
            sender.sendMessage(MessageUtils.format("&7等级: " + grade.getDisplayName()));
            sender.sendMessage(MessageUtils.format("&7分数范围: &f" + grade.getMinScore() + " - " + grade.getMaxScore()));
            sender.sendMessage(MessageUtils.format("&7描述: &f" + grade.getChineseName()));
            sender.sendMessage(MessageUtils.format("&7基础年利率: &6" + String.format("%.2f%%", grade.getBaseInterestRate() * 100)));
            sender.sendMessage(MessageUtils.format("&7最高信用额度: &6💰" + String.format("%,.0f", grade.getMaxCreditLimit())));
            
            if (sender.hasPermission("yae.admin.credit")) {
                sender.sendMessage(MessageUtils.format("&e管理员信息:"));
                sender.sendMessage(MessageUtils.format("&7• 是否符合信用贷款: " + (grade.qualifiesForLoan(com.yae.api.credit.LoanType.CREDIT) ? "&a是" : "&c否")));
                sender.sendMessage(MessageUtils.format("&7• 是否符合抵押贷款: " + (grade.qualifiesForLoan(com.yae.api.credit.LoanType.MORTGAGE) ? "&a是" : "&c否")));
                sender.sendMessage(MessageUtils.format("&7• 是否符合商业贷款: " + (grade.qualifiesForLoan(com.yae.api.credit.LoanType.BUSINESS) ? "&a是" : "&c否")));
            }
        }).exceptionally(ex -> {
    ...
    private boolean showCreditScore(@NotNull CommandSender sender, @NotNull String playerName) {
        ...
        creditService.getCreditScore(target.getUniqueId()).thenAccept(score -> {
            creditService.getCreditGrade(target.getUniqueId()).thenAccept(grade -> {
                // Calculate rank percentile
                calculateCreditRank(target.getUniqueId()).thenAccept(rank -> {
                    
                    if (sender.equals(target)) {
                        // Show to the player themselves
                        sender.sendMessage(MessageUtils.success("信用评分查询结果:"));
                        sender.sendMessage(MessageUtils.format(""));
                        sender.sendMessage(MessageUtils.format("&6━━━━━━━━━━ 您的信用信息 ━━━━━━━━━━"));
                        sender.sendMessage(MessageUtils.format("&7信用评分: &f" + score));
                        sender.sendMessage(MessageUtils.format("&7信用等级: " + grade.getDisplayName()));
                        sender.sendMessage(MessageUtils.format("&7等级描述: &f" + grade.getChineseName()));
                        sender.sendMessage(MessageUtils.format("&7分数范围: &f" + grade.getMinScore() + " - " + grade.getMaxScore()));
                        sender.sendMessage(MessageUtils.format("&7最高信用额度: &6💰" + String.format("%,.0f", grade.getMaxCreditLimit())));
                        sender.sendMessage(MessageUtils.format("&7基础年利率: &f" + String.format("%.2f%%", grade.getBaseInterestRate() * 100)));
                        
                        if (rank > 0) {
                            sender.sendMessage(MessageUtils.format("&7信用排名: &f" + String.format("%.1f%% (前%s)", rank, (100.0 - rank)) + "%"));
                        }
                        
                        // Qualification summary
                        sender.sendMessage(MessageUtils.format(""));
                        sender.sendMessage(MessageUtils.format("&6━━━━━━━━ 贷款申请资格 ━━━━━━━━"));
                        ...
                        sender.sendMessage(MessageUtils.format("&7玩家: &f" + playerName));
                        sender.sendMessage(MessageUtils.format("&7信用评分: &f" + score));
                        sender.sendMessage(MessageUtils.format("&7信用等级: " + grade.getDisplayName()));
                        sender.sendMessage(MessageUtils.format("&7等级描述: &f" + grade.getChineseName()));
                        sender.sendMessage(MessageUtils.format("&7分数范围: &f" + grade.getMinScore() + " - " + grade.getMaxScore()));
                        sender.sendMessage(MessageUtils.format("&7最高信用额度: &6💰" + String.format("%,.0f", grade.getMaxCreditLimit())));
                        sender.sendMessage(MessageUtils.format("&7基础年利率: &f" + String.format("%.2f%%", grade.getBaseInterestRate() * 100)));
                        sender.sendMessage(MessageUtils.format("&7最高可获贷款: &6💰" + String.format("%,.0f", grade.getMaxCreditLimit())));
                        
                        if (rank > 0) {
                            sender.sendMessage(MessageUtils.format("&7信用排名: " + String.format("%.1f%% (前%s)", rank, (100.0 - rank)) + "%"));
                        }
                        
                        if (sender.hasPermission("yae.admin.credit.detailed")) {
                            sender.sendMessage(MessageUtils.format(""));
                            sender.sendMessage(MessageUtils.format("&e管理员信息:"));
                            sender.sendMessage(MessageUtils.format("&7• UUID: &f" + target.getUniqueId()));
                            sender.sendMessage(MessageUtils.format("&7• 最新更新时间: &f" + "待实现")); // TODO: Add last update time
                        }
                        
                        sender.sendMessage(MessageUtils.format(""));
                        sender.sendMessage(MessageUtils.format("&7贷款资格摘要:"));
                        showQualificationSummary(sender, score, grade);
                    }
                    
                }).exceptionally(ex -> {
                    sender.sendMessage(MessageUtils.format("&7信用排名: &f计算失败"));
                    return null;
                });
    ...
    private CompletableFuture<Double> calculateCreditRank(UUID playerId) {
        return creditService.getCreditScore(playerId).thenApply(score -> {
            ...
            try {
                ...
                Logging.log(Level.WARNING, "Failed to calculate credit rank for " + playerId, e);
    ...
    private void showQualificationSummary(CommandSender sender, int score, CreditGrade grade) {
        sender.sendMessage(MessageUtils.format("&7信用贷款: " + (grade.qualifiesForLoan(com.yae.api.credit.LoanType.CREDIT) ? "&a✓ 符合" : "&c✗ 不符合")));
        sender.sendMessage(MessageUtils.format("&7抵押贷款: " + (grade.qualifiesForLoan(com.yae.api.credit.LoanType.MORTGAGE) ? "&a✓ 符合" : "&c✗ 不符合")));
        sender.sendMessage(MessageUtils.format("&7商业贷款: " + (grade.qualifiesForLoan(com.yae.api.credit.LoanType.BUSINESS) ? "&a✓ 符合" : "&c✗ 不符合")));
        sender.sendMessage(MessageUtils.format("&7应急贷款: " + (score >= 500 ? "&a✓ 符合" : "&c✗ 不符合")));
        
        if (sender.hasPermission("yae.admin.credit.detailed")) {
            sender.sendMessage(MessageUtils.format("&7预估最高额度:"));
            sender.sendMessage(MessageUtils.format("  &7• 信用贷款: &6💰" + String.format("%,.0f", grade.getMaxCreditLimit())));
            sender.sendMessage(MessageUtils.format("  &7• 抵押贷款: &6💰" + String.format("%,.0f", grade.getMaxCreditLimit() * 2)));
            sender.sendMessage(MessageUtils.format("  &7• 商业贷款: &6💰" + String.format("%,.0f", grade.getMaxCreditLimit() * 1.5)));
            sender.sendMessage(MessageUtils.format("  &7• 应急贷款: &6💰" + String.format("%,.0f", Math.min(grade.getMaxCreditLimit() * 0.3, 50000))));
        }
    }
    ...
    private boolean refreshCreditScore(@NotNull CommandSender sender, @NotNull UUID playerId, @NotNull String playerName) {
        ...
        creditService.calculateCreditScore(playerId).thenAccept(newScore -> {
            sender.sendMessage(MessageUtils.success("信用评分重算完成！"));
            showCreditScore(sender, playerName);
            
        }).exceptionally(ex -> {
            sender.sendMessage(MessageUtils.error("重新计算信用评分失败: " + ex.getMessage()));
            Logging.log(Level.WARNING, "Failed to refresh credit score for " + playerId, ex);
            return null;
        });
    ...
    private boolean showCreditHistory(@NotNull CommandSender sender, @NotNull UUID playerId, @NotNull String playerName) {
        ...
        showCreditScore(sender, playerName);
        sender.sendMessage(MessageUtils.format("&7信用历史: &f功能开发中..."));
        
        return true;
    }
    
    private boolean showHelp(@NotNull CommandSender sender) {
        sender.sendMessage(MessageUtils.format("&6━━━━━━━━━━ 信用系统命令帮助 ━━━━━━━━━━"));
        sender.sendMessage(MessageUtils.format("&e/yae credit score [player] &7- 查看信用评分"));
        
        if (sender.hasPermission("yae.command.credit.grade") || sender.hasPermission("yae.admin.credit")) {
            sender.sendMessage(MessageUtils.format("&e/yae credit grade &7- 查看信用等级详情"));
        }
        
        if (sender.hasPermission("yae.admin.credit.check")) {
            sender.sendMessage(MessageUtils.format("&e/yae credit check <player> &7- 检查指定玩家信用"));
        }
        
        if (sender.hasPermission("yae.admin.credit.refresh")) {
            sender.sendMessage(MessageUtils.format("&e/yae credit refresh [player] &7- 重新计算信用评分"));
        }
        
        if (sender.hasPermission("yae.admin.credit.history")) {
            sender.sendMessage(MessageUtils.format("&e/yae credit history [player] &7- 查看信用历史"));
        }
        
        sender.sendMessage(MessageUtils.format("&e/yae credit help &7- 显示此帮助信息"));
        sender.sendMessage(MessageUtils.format("&6━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"));
        
        // Admin summary
        if (sender.hasPermission("yae.admin.credit")) {
            sender.sendMessage(MessageUtils.format(""));
            sender.sendMessage(MessageUtils.format("&c管理员权限提醒: &7您拥有信用系统的管理员权限"));
            sender.sendMessage(MessageUtils.format("&7可以查看所有玩家的信用信息并执行管理操作"));
        }
